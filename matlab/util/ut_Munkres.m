function [assignment, cost] = ut_Munkres(costMat)
% MUNKRES   Munkres (Hungarian) Algorithm for Linear Assignment Problem. 
%
% [ASSIGN,COST] = ut_Munkres(COSTMAT) returns the optimal column indices,
...
%{
[assignment,cost] = ut_Munkres(magic(5));
...
% Modifications by Shyam Madhusudhana, May 2015.
% https://github.com/shyamblast/sound-detectors
%  - Changed the return 'assignment' to type uint16.
%  - Returning 'assignment' as a column vector to match the input.
%  - Some speed optimisation in outerplus().

assignment = zeros(size(costMat,1), 1, 'uint16');
cost = 0;
...
% Cost of assignment
rowIdx = uint16(find(validRow));
colIdx = uint16(find(validCol));
starZ = starZ(1:nRows);
vIdx = starZ <= nCols;
assignment(rowIdx(vIdx)) = colIdx(starZ(vIdx));
pass = assignment(assignment>0);
pass(~diag(validMat(assignment>0,pass))) = uint16(0);
...
for c=1:ny
    M_copy(:,c)=M(:,c)-(x+y(c));
    minval = min(minval,min(M_copy(:,c)));
end
[rIdx,cIdx]=find(M_copy==minval);
